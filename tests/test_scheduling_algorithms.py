...
def test_ids_and_names_are_stable():
    ...
    assert ALGORITHMS[1] is ExtintionMonitor and ExtintionMonitor.name() == "STD"
...
def _make_block(session, pid="P01", sched_algorithm=3):
    target = model.Targets(name="obj", target_ra=10.0, target_dec=0.0)
    session.add(target)
    session.commit()
...
def test_recurrent_add_and_observed_round_trip(session_factory):
    ...
    assert recurrent.tid == target.id
